...
class WildDomainAdaptationModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        copyfconn=copy.deepcopy(self.netG_BC.fconn)
        copyscore_fconn=copy.deepcopy(self.netG_BC.score_fconn)
        self.netG_AMidAhead=torch.nn.Sequential(copyfeat4,copyfeat5,copyfconn,copyscore_fconn)
        self.netG_AMid=torch.nn.Sequential(self.netG_BC.feats,self.netG_AMidAhead[0],self.netG_AMidAhead[1],self.netG_AMidAhead[2],self.netG_AMidAhead[3])
        self.netG_BMid=torch.nn.Sequential(self.netG_BC.feats,self.netG_BC.feat4,self.netG_BC.feat5,self.netG_BC.fconn,self.netG_BC.score_fconn)
        self.netG_AC=  network_classification_model('FCN16',self.gpu_ids,feats=self.netG_AMid[0],feat4=self.netG_AMid[1],feat5=self.netG_AMid[2],fconn=self.netG_AMid[3], score_fconn=self.netG_AMid[4],score_feat4=self.netG_BC.score_feat4,num_classes=self.opt.num_classes)
        #self.netG_AC= torch.nn.Sequential(self.netG_AMid,self.netG_BC.fconn,self.netG_BC.score_fconn,self.netG_BC.score_feat4)
        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD_AB = networks.define_D(21, opt.ndf,
        ...
        if not self.isTrain or opt.continue_train:
            ...
            if opt.switch==0:
                ...
                self.netG_AMidAhead=torch.nn.Sequential(copyfeat4,copyfeat5,copyfconn,copyscore_fconn)
                self.netG_AMid=torch.nn.Sequential(self.netG_BC.feats,self.netG_AMidAhead[0],self.netG_AMidAhead[1],self.netG_AMidAhead[2],self.netG_AMidAhead[3])
                self.netG_AC=  network_classification_model('FCN16',self.gpu_ids,feats=self.netG_AMid[0],feat4=self.netG_AMid[1],feat5=self.netG_AMid[2],fconn=self.netG_AMid[3], score_fconn=self.netG_BC.score_fconn,score_feat4=self.netG_BC.score_feat4,num_classes=self.opt.num_classes)
                if self.isTrain:
                    self.netD_AB = networks.define_D(21, opt.ndf,
                                             opt.which_model_netD,
                                             opt.n_layers_D, use_sigmoid, self.gpu_ids)
    ...
    def optimize_parameters(self):
        ...
        if self.inputType in 'AB':
            ...
            for i in range(self.opt.critic_freq):
    ...
    def backward_G(self,rA,rB):
        ...
        self.loss_G_A_false = self.criterionGAN(pred_false, True)
        
        pred_true=self.netD_AB.forward(self.rBfc7)
